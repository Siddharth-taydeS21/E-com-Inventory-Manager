...
const validateFrom = (FormContainer) => {
    ...
    const imageInput = FormContainer.querySelector('#product_image');
    ...
    if (!imageInput.files || imageInput.files.length === 0) {
        ...
        showErrorMsg(ImageParentElem.parentElement, 'Invalid file type. Only JPG, PNG, WEBP, and GIF are allowed');
        validateImage = false;
    ...
    //category validation 
    const categoryParentEl = document.querySelector('.category_div').parentElement;
    ...
    // if input parent element classname includes hidden, return;
    // else, do this all - 
    ...
    if (maxSevenRegex.test(tagsInput.value)) {
        ...
        showErrorMsg(tagsInput.parentElement, 'Please provide at least than 7 tags');
        validateTags = false;
    }
